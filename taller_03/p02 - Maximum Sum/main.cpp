...
void build()
{
    ...
    for (int i = n - 1; i > (n - 1)>>1; i--) s[i] = std::min(s[i<<1], s[i<<1|1]);
...
void update(int p, int value) // set value at position p
{
    ...
    for (t[p] = value, s[p] = value, p >>= 1; p > 0; p >>= 1)
    {
        ...
        if (p > (n - 1)>>1) s[p] = std::min(s[p<<1],s[p<<1^1]);
        else
        {
...
{  // greatest possible sum on interval [l, r]
    ...
    {
        if (bool(l&1)) max = std::max(t[l++], max);
        if (bool(r&1)) max = std::max(t[--r], max);
    }
    return max;
}
...
// second_highest value on interval [l, r]
{
    ...
    {
        if (bool(l&1)) {
            if (r >= n) posibleValues.push_back(t[l++]);
            else {
        ...
        if (bool(r&1))
            if (r >= n) posibleValues.push_back(t[--r]);
            else {
