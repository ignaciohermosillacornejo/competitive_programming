...
class SegTreeLazy
{
    ...
    intt flip(intt u)
    {
        if (lazy[u] == 0) return 3; // nothing with a flip turns into a flip
        if (lazy[u] == 1) return 2; // a flip of a set 1 turns into a set 0
        if (lazy[u] == 2) return 1; // a flip of a set 0 turns into a set 1
        if (lazy[u] == 3) return 0; // a flip + flip turns into nothing
    }

    void propagate(int u, int i, int j, intt x)
    {
        if (x == 0) return; // nothing to propagate
